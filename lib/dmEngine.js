...
let _running  = false;
let _stopFlag = false;
let _safetyComponents = {};  // External safety components
...
function generateHumanDelay() {
  // Weighted distribution: 40% short (5-8s), 40% medium (8-12s), 20% long (12-20s)
...
function assessDetectionRisk(accountId, stats) {
  ...
  // Pattern detection: too many rapid sends
  if (stats.messagesBurst > 3) {
    risks.push({ level: 'high', reason: 'Message burst detected' });
  }
  
  // Identical message to many users
  if (stats.identicalMessageCount > 2) {
    risks.push({ level: 'high', reason: 'Identical message pattern' });
  }
  
  // Time-based pattern (always sending at same time)
  if (stats.timingPattern === 'consistent') {
    risks.push({ level: 'medium', reason: 'Consistent timing pattern' });
  }
