...
function igLogin(acc) {
  if (!sessionFileExists(acc.username)) {
    return {
      ok: false,
      error: `No session file for @${acc.username}`,
      reason: 'no_session'
    };
  }

  addLog(`[SESSION MODE] Using session for @${acc.username}`, null, acc.id);

  return dispatch({ 
    cmd: 'login', 
    username: acc.username, 
    password: '', // FORCE session mode
    session_path: getSessionPath(acc.username)
  });
}

async function igSearch(acc, keyword) {
  let password = ''; // FORCE SESSION MODE ONLY
...
if (!sessionFileExists(acc.username)) {
  addLog(`❌ No session file for @${acc.username}`, campaignId, acc.id, 'error');
  continue;
}

addLog(`[SESSION MODE ONLY] @${acc.username}`, campaignId, acc.id);
...
try {
  ...
  addLog(`CRASH prevented for @${acc.username}: ${e.message}`, campaignId, acc.id, 'error');
  continue;
}
      if (r.ok) {
