...
// Strip SQL modifiers that don't affect column matching (COLLATE, NOCASE, etc.)
function stripModifiers(clause) {
  return clause.replace(/\s+COLLATE\s+\w+/gi, '');
}

function parseWhere(whereClause, params) {
  if (!whereClause) return () => true;
  // Strip unsupported modifiers before parsing
  const cleanClause = stripModifiers(whereClause);
  let idx = 0;
  const conditions = cleanClause.split(/\s+AND\s+/i).map(cond => {
    ...
    if (eqMatch) {
      ...
      return row => String(row[col] || '').toLowerCase() === String(val || '').toLowerCase();
    }
  ...
  return {
    run(...args) {
      ...
      if (sqlU.startsWith('INSERT')) {
        ...
        // Build row — if 'id' is explicitly in columns (UUID primary key), use it;
        // otherwise assign an auto-increment integer id.
        const row = {};
        cols.forEach((col, i) => {
      ...
      if (sqlU.startsWith('UPDATE')) {
        ...
        for (const part of setParts) {
          ...
          // Only match bare literal 0/1 — must not be followed by more word chars
          const val0Match = part.match(/^(\w+)\s*=\s*0\s*$/);
          const val1Match = part.match(/^(\w+)\s*=\s*1\s*$/);
