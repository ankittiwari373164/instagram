...
for (const [key, value] of Object.entries(state.cookies)) {
  await jar.setCookie(
    ...
    console.log('[ig_client] Cookies injected');

    // ✅ Set user id
    if (state.authorization_data?.ds_user_id) {
      ig.state.cookieUserId = state.authorization_data.ds_user_id;
    }
    ...
    console.log(`[ig_client] ✅ Logged in as @${user.username}`);
    return ig;
