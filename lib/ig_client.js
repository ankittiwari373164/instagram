 ...
 * AUTO DETECTS SESSION FILES + OPTIMIZED FOR MANUAL SESSION UPLOADS
...
async function cmdLogin({ username, password }) {
  try {
    ...
    // ── STRATEGY 1: Check for session file first ──────────────────
    const sf = sessionFile(clean);
    if (fs.existsSync(sf)) {
      console.log(`[ig_client] Session file found for @${clean}, attempting to load...`);
      try {
        const ig = await getClientFromSession(clean);
        ...
        console.warn(`[ig_client] Session load failed: ${sessionError.message}, trying password...`);
        // Fall through to password login
      }
    }
    
    // ── STRATEGY 2: Check if password is actually "session_X" ─────
    if (password && password.startsWith('session_')) {
      console.log(`[ig_client] Detected session indicator in password field, looking for session file...`);
      try {
        const ig = await getClientFromSession(clean);
        const info = await ig.account.currentUser();
        return {
        ...
        console.warn(`[ig_client] Session load failed: ${e.message}`);
      }
    }
    
    // ── STRATEGY 3: Try password login ─────────────────────────────
    if (!password || password.trim() === '') {
      return {
        ok: false,
        error: `No password provided and no session file found for @${clean}`,
        reason: 'no_auth_method'
      };
    }
    
    console.log(`[ig_client] Logging in @${clean} with password`);
    const ig = new IgApiClient();
    await sleep(jitter(2000, 4000));
    await ig.account.login(clean, password);
    ...
    try {
      ...
      fs.writeFileSync(sessionFile(clean), JSON.stringify(state, null, 2));
      console.log(`[ig_client] Session saved for @${clean}`);
    } catch (e) {
...
async function cmdSearch({ username, password, keyword }) {
  try {
    ...
    let ig;
    if (!password || password.trim() === '') {
      ig = await getClientFromSession(username.toLowerCase());
    } else if (password.startsWith('session_')) {
      ig = await getClientFromSession(username.toLowerCase());
    } else {
      ig = new IgApiClient();
      await sleep(jitter(2000, 4000));
      await ig.account.login(username.toLowerCase(), password);
    }
...
async function cmdCheckSession({ username }) {
  const sf = sessionFile(username.toLowerCase());
  if (!fs.existsSync(sf)) {
    return { 
      ok: false, 
      valid: false, 
